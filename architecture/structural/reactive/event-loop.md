...
**1 本（または少数）のイベントループが、  
非同期 I/O とコールバック／ハンドラを順に処理していく構造スタイル。**  
...
こうした処理は、ワーカー（別スレッド／プロセス）に逃がす必要があります。
...
## 8. Framework Examples（代表的なフレームワーク・実行環境）

Event Loop ベース構造は、非同期 I/O を中心としたプラットフォームで広く採用されています。
...
Event Loop 自体は低レイヤの実行モデルだが、次のパターンと強く結びつきます。
...
というシンプルな考え方で高スループットを実現するスタイルです。

I/O バウンドなサーバを設計するときの、  
**“デフォルト候補” の一つとして検討する価値があります。**
