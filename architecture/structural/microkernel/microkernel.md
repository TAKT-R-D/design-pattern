...
## 8. Framework Examples（代表的なフレームワーク・プロダクト）

Microkernel Architecture は OS だけでなく、多くの「拡張性を核とするプロダクト」で採用されています。
...
Microkernel は明確に複数パターンの組み合わせによって成立しているスタイルです。
...
において、**「変わらない部分」と「変わる部分」を構造的に分離するための強力なスタイル** です。

ただし、小規模なアプリに無理に適用するとオーバーエンジニアリングになりやすいため、
適用範囲と寿命を見極めて採用することが重要です。
